...
namespace Lolly {
  ...
  int SimpleMemoryPool::_decideGranuality(int request_gran) {
    ...
    if (max_granuality < result_granuality) {
      std::cout << "Request granuzality exceeds max granuality allowed!" << std::endl;
  ...
  void *SimpleMemoryPool::Get(size_t request_sz) {
    ...
    if (0 == continuous_number || -1 == start_block_idx || continuous_number < need_block_num) {
      std::cerr << "Cannot find enough continuous free blocks for the request size" << std::endl;
      return nullptr;
    }

    for (int idx = start_block_idx; idx < start_block_idx + continuous_number; idx++) {
  ...
  void SimpleMemoryPool::Put(void *data) {
    ...
    for (int start_idx = result_idx; start_idx < result_idx + continual_number; start_idx++) {
