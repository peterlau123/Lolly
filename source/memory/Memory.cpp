...
SimpleMemoryPool::SimpleMemoryPool(size_t byte_count, int granuality = 64) {
  ...
  pool_data_ = std::unique_ptr<void *>(data, [](void *p) {
    if (nullptr != p) {
      free(p);
    }
  });
  ...
  for (int index = 0; index < final_block_num; index++) {
    Block block(data + i * final_granuality, granuality_);
...
void SimpleMemoryPool::Put(void *data) {
  ...
  int result_idx = -1;
  for (size_t i = o; i < blocks_.size(); i++) {
